...
class Subheading_Dataset(Dataset):
    ...
    def mask_tokens(self, inputs: torch.Tensor, special_tokens_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
            ...
            # Sampling of few tokens for mlm probability calculation (with probability `self.mlm_probability`)
            ...
            # Replace masked input tokens with tokenizer input
            ...
            # Replacing masked input tokens with random inputs
            ...
            # Keeping the masked input tokens unchanged
            return inputs
